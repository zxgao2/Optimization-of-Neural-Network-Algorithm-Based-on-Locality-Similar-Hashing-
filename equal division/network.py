       ...
       # print begin_end
        for j in xrange(epochs):
             ...
             for i in range (max_index):
                    ...
                    mini_batches1.append(mini_batch1)

             for mini_batch in mini_batches1:
        ...
        plt.title("equal-eta {0}".format(eta)) #图标题
        plt.savefig("./result1/eqity-eta3 {0}.jpg".format(eta)) #保存图
        plt.figure(figsize=(8,4)) #创建绘图对象
        plt.plot(x,f,"b--",linewidth=1)   #在当前绘图对象绘图（X轴，Y轴，蓝色虚线，线宽度）
        plt.xlabel("Epoch") #X轴标签
        plt.ylabel("E")  #Y轴标签
        plt.title("E equal-eta {0}".format(eta)) #图标题
        plt.savefig("./result/E Primary-eta3 {0}.jpg".format(eta)) #保存图
