 ...
 #       print activations[-1]          
    ...
    def SGD(self,training_data,epochs,mini_batch_size,eta,test_data = None):
        ...
        for j in xrange(epochs):
            ...
            # mini_batches = [training_data[0:0+n-1]]
            ...
            if test_data:
                ...
                print "Epoch {0}:{1}/{2} {3}".format(j,self.evaluate(test_data),n_test,float(k)/n_test)
            else:
        ...
        plt.title("Primary {0}".format(eta)) #图标题
        plt.savefig("./result/Primary-eta-sat {0}.jpg".format(eta)) #保存图
        plt.figure(figsize=(8,4)) #创建绘图对象
        plt.plot(x,f,"b--",linewidth=1)   #在当前绘图对象绘图（X轴，Y轴，蓝色虚线，线宽度）
        plt.xlabel("Epoch") #X轴标签
        plt.ylabel("E")  #Y轴标签
        plt.title("E Primary-eta {0}".format(eta)) #图标题
        plt.savefig("./result/E Primary-eta4-sat {0}.jpg".format(eta)) #保存图
