...
class DensityPeakCluster(object):
	def local_density(self, load_func, distance_f, dc = None, auto_select_dc = False):
        ...
        #print max_id,max_dis,min_dis,distances
		if dc == None:
	...
	def cluster(self, load_func, distance_f, density_threshold, distance_threshold, dc = None, auto_select_dc = False):
		...
		distance_threshold1=0
		for idx, (ldensity, mdistance, nneigh_item) in enumerate(zip(rho, delta, nneigh)):
			density_threshold1=density_threshold1 + ldensity
			distance_threshold1=distance_threshold1 + mdistance
		density_threshold1=density_threshold1 / len(rho)
		distance_threshold1=distance_threshold1 / len(rho)
		print distance_threshold1
		for idx, (ldensity, mdistance, nneigh_item) in enumerate(zip(rho, delta, nneigh)):
			if idx == 0: continue
			print 'lm',idx,ldensity,mdistance
			if ldensity >= density_threshold1 and mdistance >= distance_threshold1:
				ccenter[idx] = idx
				cluster[idx] = idx
			else:
				print '---1',idx
