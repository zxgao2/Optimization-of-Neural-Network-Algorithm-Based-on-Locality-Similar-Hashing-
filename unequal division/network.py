...
class Network(object):
    ...
    def SGD(self,training_data,epochs,begin_end,index,eta,test_data = None):
        if test_data:
       ...
       # print begin_end
        for j in xrange(epochs):
             ...
             if test_data:
                ...
                #print f
                x.append(j)
                y.append(float(k)/n_test)
                print "Epoch {0}:{1}/{2}  {3}".format(j,k,n_test, float(k)/n_test)
             else:
        ...
        plt.title("unequal-eta {0}".format(eta)) #图标题
        plt.savefig("./result/unequal-eta5-sat {0}.jpg".format(eta)) #保存图
        plt.figure(figsize=(8,4)) #创建绘图对象
        plt.plot(x,f,"b--",linewidth=1)   #在当前绘图对象绘图（X轴，Y轴，蓝色虚线，线宽度）
        plt.xlabel("Epoch") #X轴标签
        plt.ylabel("E")  #Y轴标签
        plt.title("E unequal-eta {0}".format(eta)) #图标题
        plt.savefig("./result/E unequal-eta5-sat {0}.jpg".format(eta)) #保存图
